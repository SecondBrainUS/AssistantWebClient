<template>
  <div class="spotify-card rounded-lg overflow-hidden">
    ...
    <!-- Custom Card View -->
    <div v-if="isCustomView && userData" class="p-4 bg-[#191414]">
      <div class="flex gap-4">
        <!-- User Avatar -->
        <div class="flex-shrink-0 bg-gray-800 rounded-full h-16 w-16 flex items-center justify-center overflow-hidden">
          <img v-if="userImage" :src="userImage" alt="Profile Picture" class="w-full h-full object-cover" />
          <svg v-else width="32" height="32" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <path d="M12 13C8.13401 13 5 16.134 5 20V21H19V20C19 16.134 15.866 13 12 13Z" fill="#9CA3AF" />
          </svg>
        </div>
        
        <!-- User Info -->
        <div class="flex flex-col">
          <h3 class="font-medium text-white text-lg">{{ userData.display_name }}</h3>
          <p class="text-gray-400 text-sm">{{ userData.id }}</p>
          
          <!-- User details -->
          <div class="grid grid-cols-2 gap-x-4 gap-y-1 mt-2">
            <div class="text-gray-400 text-sm">Email:</div>
            <div class="text-gray-300 text-sm">{{ userData.email || 'Not available' }}</div>

            <div class="text-gray-400 text-sm">Country:</div>
            <div class="text-gray-300 text-sm">{{ userData.country || 'Not available' }}</div>
            
            <div class="text-gray-400 text-sm">Subscription:</div>
            <div class="text-gray-300 text-sm capitalize">{{ userData.product || 'Not available' }}</div>
            
            <div class="text-gray-400 text-sm">Followers:</div>
            <div class="text-gray-300 text-sm">{{ userData.followers?.total || 0 }}</div>
          </div>
          
          <!-- Spotify Link Button -->
          <a 
            v-if="userData.external_urls?.spotify" 
            :href="userData.external_urls.spotify" 
            target="_blank" 
            class="mt-3 inline-block w-fit px-3 py-1 bg-[#1DB954] text-black text-sm font-medium rounded-full hover:bg-opacity-90 transition-colors"
          >
            Open in Spotify
          </a>
        </div>
...
const userImage = computed(() => {
  if (!userData.value || !userData.value.images) return null;
  return userData.value.images.length > 0 ? userData.value.images[0].url : null;
});
